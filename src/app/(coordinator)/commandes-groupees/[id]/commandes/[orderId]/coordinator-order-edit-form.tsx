...
export function CoordinatorOrderEditForm({
  groupOrderId,
  memberOrderId,
  products,
  deliveryPoints,
  ...
  async function submit() {
    if (lines.length === 0) {
      setError("Ajoutez au moins un produit, ou supprimez la commande si plus rien n'est disponible.")
      return
    }
    ...
    const res = await fetch(`/api/coordinator/member-orders/${memberOrderId}`, {
      method: "PATCH",
    ...
    router.push(`/commandes-groupees/${groupOrderId}`)
    router.refresh()
  }

  async function deleteOrder() {
    if (!confirm("Supprimer définitivement cette commande membre ?")) return

    const res = await fetch(`/api/coordinator/member-orders/${memberOrderId}`, { method: "DELETE" })
    if (res.ok) {
      router.push(`/commandes-groupees/${groupOrderId}`)
      router.refresh()
    }
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
      <div className="p-3 bg-blue-50 text-blue-700 rounded-lg text-sm">
        Modification en tant que coordinateur — utile en cas de non-fourniture d&apos;un produit par le producteur.
      </div>
      ...
      {paymentReferents.length > 0 && (
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Référent paiement</label>
          <select
